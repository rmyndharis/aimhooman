...
test('lifecycle queue serializes init/init, init/uninstall, and failed/successful init pairs', async (t) => {
    ...
    const waitForCandidate = (lock, child) => {
        ...
        assert.ok(candidate, `timed out waiting for lifecycle candidate from PID ${child.pid}`);
        return candidate;
    };
    ...
    const launch = (repo, args) => spawn(process.execPath, [CLI, ...args], {
        cwd: repo.root,
        env: repo.env,
...
`);
        return spawn(process.execPath, [script, lock, exclude], {
            cwd: repo.root,
            env: repo.env,
            stdio: ['ignore', 'pipe', 'pipe'],
        });
