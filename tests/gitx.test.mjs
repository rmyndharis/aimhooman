...
test('trackedEntries retains every blob candidate for an unresolved conflict', () => {
    ...
    try {
        ...
        execFileSync('git', ['checkout', '-q', '-b', 'marker-side'], { cwd: dir });
        writeFileSync(join(dir, 'conflict.txt'), '// generated by AI\n');
        execFileSync('git', ['add', 'conflict.txt'], { cwd: dir });
        commit(dir, 'marker side');

        execFileSync('git', ['checkout', '-q', main], { cwd: dir });
        writeFileSync(join(dir, 'conflict.txt'), 'safe side\n');
        execFileSync('git', ['add', 'conflict.txt'], { cwd: dir });
        commit(dir, 'safe side');
        assert.throws(() => execFileSync('git', ['merge', 'marker-side'], {
            cwd: dir,
        ...
        assert.ok(scan.findings.some((finding) => finding.ruleId === 'marker.ai-authored'));
    } finally {
        rmSync(dir, { recursive: true, force: true });
    }
});
...
test('tracked entries leave gitlinks out of the cat-file metadata read', () => {
    ...
    // OpenSSL field run — its 11 submodule pins killed `audit` on a blob:none
    // clone.
